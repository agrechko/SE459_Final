...
public class ExploringState implements RobotStates {
	...
	public void execute(RobotController robot) {
		if (robot.firstStart) {
			...
			int[] paths = robot.sensors
					.getPaths(robot.currentX, robot.currentY);// this is needed
																// so that
																// sensors scan
																// the current
																// cell for info
			...
			if (robot.currentPower <= (robot.maxPower / 2)) {
				...
				if (paths[0] == 1 && robot.wentBackFrom != 0
						&& (cameFrom == -1 || cameFrom != 1))// can go x pos and
																// did not come
																// from x neg
				{
					pathsIndexes.add(0);
				...
				if (paths[1] == 1 && robot.wentBackFrom != 1
						&& (cameFrom == -1 || cameFrom != 0))// can go x neg and
																// did not come
																// from x pos
				{
					pathsIndexes.add(1);
				...
				if (paths[2] == 1 && robot.wentBackFrom != 2
						&& (cameFrom == -1 || cameFrom != 3))// can go y pos and
																// did not come
																// from y neg
				{
					pathsIndexes.add(2);
				...
				if (paths[3] == 1 && robot.wentBackFrom != 3
						&& (cameFrom == -1 || cameFrom != 2))// can go y neg and
																// did not came
																// from y pos
				{
					pathsIndexes.add(3);
				...
				if (pathsIndexes.size() == 0) // need to go back no way out
				{
					...
					while (!selected && dirtyPaths.size() > 0)// pick a random
																// unvisited
																// path to go to
					{
						int selectedIndex = gen.nextInt(dirtyPaths.size());
						selectedPath = dirtyPaths.get(selectedIndex);// gets a
																		// random
																		// unvisited
																		// path
					...
					while (!selected && pathsIndexes.size() > 0) {
						...
						floorType = getFloorType(robot, selectedPath);// robot.sensors.getSurface(robot.currentX,
																		// robot.currentY);
						...
						pathsIndexes.remove(selectedIndex);// remove index
															// because moving to
															// this path will
															// draw to much
															// power
				...
				if (robot.currentState != State.GOING_HOME.getValue()) {
					...
					if (robot.sensors.getCell(robot.currentX, robot.currentY)
							.isChargingStation()) {
						robot.route.clear();// cleans back route stack if we
											// encountered a charging station
											// because we know a shorter way
											// back home
					}
	...
	private boolean getCleanStatus(RobotController robot, int i) {
		...
		switch (i) {
		case 0:
			robot.sensors.getCell(robot.currentX + 1, robot.currentY);// gets
																		// basic
																		// information
																		// about
																		// adjacent
																		...
																		// into
																		// memory
			status = robot.sensors.isClean(robot.currentX + 1, robot.currentY);// checks
																				// to
																				// see
																				// if
																				// the
