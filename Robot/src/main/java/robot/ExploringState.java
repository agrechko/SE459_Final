...
public class ExploringState implements RobotStates {
    ...
    private void choosePathAndMove(final RobotController robot) {
        ...
        if (pathsIndexes.isEmpty()) {
            goBackOneStep(robot);
            floorType = robot.sensors
                    .getSurface(robot.currentX, robot.currentY);
        } else {
    ...
    private void pickPath(final RobotController robot,
            final ArrayList<Integer> paths) {
    ...
    private void populatePaths(final RobotController robot, final int[] paths,
            final ArrayList<Integer> pathsIndexes, final int cameFrom) {
    ...
    private void move(final RobotController robot, final int selectedPath) {
        switch (selectedPath) {
        case 0:
            robot.currentX += 1;
            robot.route.push(0);
    ...
    private boolean getCleanStatus(final RobotController robot, final int i) {
    ...
    private void goBackOneStep(final RobotController robot) {
    ...
    // power to move there
    public final int getFloorType(final RobotController robot,
