...
//        	}
    ...
    private void findClosestDirt(RobotController robot) {
		LinkedList<CellData> queque = new LinkedList<CellData>();
		Stack<CellData> dirtPath = new Stack<CellData>();
		
		//string coordinates of the current cell and the celldata of the parent cell
		...
		//string current cell reverse direction pointing to parent which leads to home
		...
		while(!queque.isEmpty() && !found){
			...
			int[] paths = robot.sensors.getPaths(cur.getCellX(), cur.getCellY());
			int pathSize = paths.length;
			for(int i = 0; i < pathSize; i++ ){
				if(paths[i] == 1){
					CellData cell = lookForCellInMemory(robot, cur, i);
					if(cell != null){
						if(!visited.containsKey(cell.getCellX()+","+cell.getCellY())){
							queque.add(cell);
							parent.put(cell.getCellX()+","+cell.getCellY(), cur);
							visited.put(cell.getCellX()+","+cell.getCellY(), cur.getCellX()+","+cur.getCellY());
							directionToParent.put(cell.getCellX()+","+cell.getCellY(), i);
						}
					}
				}
			}
		...
		robot.pathToDirtyCell = getDirtPathForCell(robot, parent, directionToParent, cur);
	}
    
    //finds the path to the dirty cell
    private LinkedList<Integer> getDirtPathForCell(RobotController robot, HashMap<String,CellData> parent, HashMap<String,Integer> directionToParent, CellData cell) {
    	LinkedList<Integer> dirtPath = new LinkedList<Integer>();
    	
    	CellData cur = cell;
    	
    	while(parent.get(cur.getCellX()+","+cur.getCellY()) != null) {
    		dirtPath.add(directionToParent.get(cur.getCellX()+","+cur.getCellY()));
    		cur = parent.get(cur.getCellX()+","+cur.getCellY());
    	}
    	//removes the last step because exploring logic should pick it up from there
    	dirtPath.pollLast();
    	return dirtPath;
    }
	
	//looks for the correct cell data based on paths array index
	private CellData lookForCellInMemory(RobotController robot, CellData cur, int i){
		CellData cell = null;
		switch (i) {
		case 0:
			cell = robot.sensors.getCellMemory(cur.getCellX() + 1, cur.getCellY());
			break;
		case 1:
			cell = robot.sensors.getCellMemory(cur.getCellX() - 1, cur.getCellY());
			break;
		case 2:
			cell = robot.sensors.getCellMemory(cur.getCellX(), cur.getCellY() + 1);
			break;
		case 3:
			cell = robot.sensors.getCellMemory(cur.getCellX(), cur.getCellY() - 1);
			break;
